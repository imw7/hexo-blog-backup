...
无论是软件开发的调试阶段还是软件上线之后的运行阶段，日志一直都是非常重要的一个环节，我们也应该养成在程序中记录日志的好习惯。<!--more-->
...
```bash
2019/06/22 19:04:17 这是一条很普通的日志。
2019/06/22 19:04:17 这是一条很普通的日志。
2019/06/22 19:04:17 这是一条会触发fatal的日志。
```
...
```go
2019/06/22 19:05:17.494943 .../log_demo/main.go:11: 这是一条很普通的日志。
```
...
```bash
[小王子]2019/06/22 19:05:57.940542 .../log_demo/main.go:13: 这是一条很普通的日志。
```
...
```bash
<New>2019/06/22 19:06:51 main.go:34: 这是自定义的logger记录的日志。
```
